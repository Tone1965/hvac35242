...
export default function LocationsFramework({ lang }: LocationsFrameworkProps) {
  ...
  const text = {
    en: {
      ...
      hideAll: "Hide All Service Areas",
    ...
    es: {
      ...
      viewAll: "Ver Todas las Áreas de Servicio",
      hideAll: "Ocultar Todas las Áreas de Servicio",
        ...
        {remainingLocations.length > 0 && (
          <div className="text-center">
            <Button
              ...
              variant="outline"
              size="lg"
              className="border-2 border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-blue-900 font-bold text-lg px-8 py-4"
            >
              <MapPin className="w-5 h-5 mr-2" />
              {showAllAreas ? currentText.hideAll : `${currentText.viewAll} (${uniqueLocations.length})`}
              {showAllAreas ? <ChevronUp className="w-5 h-5 ml-2" /> : <ChevronDown className="w-5 h-5 ml-2" />}
            </Button>
            
            {showAllAreas && (
              <div className="mt-8 bg-white/10 backdrop-blur-sm rounded-xl p-8 border border-yellow-400/30">
                <h4 className="text-2xl font-bold mb-6 text-yellow-300">{currentText.allAreas}</h4>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {remainingLocations.map((location) => (
                    <Link
                      key={location.slug}
                      href={`/${lang}/services/locations/${location.slug}`}
                      className="text-white hover:text-yellow-300 transition-colors text-sm font-medium p-2 hover:bg-white/10 rounded"
                    >
                      {location.name}
                    </Link>
                  ))}
